...
|head|文書のヘッダ情報を表す|
|meta|メタデータを指定する|
|title|文書にタイトルを付け, ブラウザのツールバーに表示される|
|body|文書の本体を表す|
|div|ひとかたまりの範囲として定義する|
|span|ひとつの範囲として定義する|
|h1〜h6|見出しであることを表す|
|p|パラグラフ(段落)であることを表し, 文章を記述する|
|br|改行する|
|hr|テーマや話題の区切りを表し, 横線が引かれる|
|ul|順序のないリストを表示する|
|ol|順序のあるリストを表示する|
|li|リストの項目を記述する|
|a|ハイパーリンクを指定する|
|img|画像を表示する|
|form|入力・送信フォームを作る|
|input|テキストボックスなどのフォームを構成する様々な入力部品を作成する|
|button|ボタンを作成する|
|label|フォームの入力部品と項目名を関連付ける|
|table|テーブル(表)を作成する|
...
|属性|説明|
|:--|:--|
|id|特定の要素へスタイルを設定するために指定する|
|class|特定の要素へスタイルを設定するために指定する|
|href|リンクされたソースのURL|
|src|画像等の埋め込みコンテンツのURLを指定する|
|action|フォーム経由で送信される情報を処理するURIを指定する|
|method|フォームを送信する際に使用するHTTPメソッドを定義する|
|for|この要素が属する要素を指定する id属性の値と紐づけられる|
|type|要素の型を定義する|
|name|要素の名前 送信されたフォームでサーバーがフィールドを識別するために使用|
|value|ページ読み込み時に要素で表示するデフォルト値を定義する|
|disabled|閲覧者によるコントロールの操作を無効にするかどうかを示す|
|charset|ページまたはスクリプトの文字エンコーディングを宣言する|

`id` 属性と `class` 属性の違いは, 属性値が一意かどうかである.  
`id` 属性は一意に指定し, `class` 属性は一意に指定しなくてもよい.
